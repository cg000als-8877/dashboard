  ...
  return (
    <div className="space-y-8 animate-[fade-up_0.4s_ease-out_both] pb-20 md:pt-0">
      ...
      <header className="flex flex-col mb-8 relative z-10">
        <h1 className="text-2xl md:text-4xl font-bold md:font-black tracking-tight text-[var(--color-text-main)]">Archive</h1>
        ...
        {ARCHIVE_MONTHS.map((month) => (
          ...
          >
            <div className="flex-1">
              <h2 className="text-lg sm:text-xl font-semibold sm:font-bold text-[var(--color-text-main)] mb-0.5">{month.name}</h2>
              <p className="text-xs sm:text-sm text-[var(--color-text-muted)]">{month.description}</p>
            </div>
            
            <div className="flex-shrink-0">
              <div className="px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg bg-[var(--color-primary)] text-[var(--color-on-primary)] text-[10px] sm:text-xs font-bold uppercase tracking-wider [box-shadow:0_0_15px_var(--color-primary-glow)] group-hover:[box-shadow:0_0_25px_var(--color-primary-glow-hover)] transition-all flex items-center gap-1.5">
