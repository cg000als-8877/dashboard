  ...
  return {
    ...
    getInsights() {
      ...
      if (stats.averageDailyProfit < 0) {
        insights.push(`<strong>DAILY CASH BURN:</strong> Current burn run-rate is <span class="text-rose-600 dark:text-rose-400 font-bold">-BDT ${Math.abs(stats.averageDailyProfit).toLocaleString()} / day</span> across <span class="text-[var(--color-primary)] font-bold">${stats.workingDays} active production day(s)</span>.`);
      } else {
        insights.push(`<strong>DAILY PROFIT RUN-RATE:</strong> Operating with an average daily profit of <span class="text-emerald-600 dark:text-emerald-400 font-bold">+BDT ${stats.averageDailyProfit.toLocaleString()} / day</span> across <span class="text-[var(--color-primary)] font-bold">${stats.workingDays} active day(s)</span>.`);
      }
      ...
      if (highestRecoveryLine && lowestRecoveryLine) {
        insights.push(`<strong>LINE EFFICIENCY SPREAD:</strong> Line recovery rates range from <span class="text-rose-600 dark:text-rose-400 font-bold">${lowestRecoveryLine.monthCostRecovery || '0.0'}% (${lowestRecoveryLine.name})</span> to <span class="text-emerald-600 dark:text-emerald-400 font-bold">${highestRecoveryLine.monthCostRecovery || '0.0'}% (${highestRecoveryLine.name})</span>, requiring floor re-balancing.`);
      }
      ...
      if (breakevenSurgeQty > 0) {
        insights.push(`<strong>BREAKEVEN OUTPUT TARGET:</strong> To fully offset daily loss at current CM rates, daily output must increase by <span class="text-emerald-600 dark:text-emerald-400 font-bold">+${breakevenSurgeQty.toLocaleString()} PCS/day</span> (+${breakevenPct}% revenue surge).`);
      }
